    ...
    <div class="py-[2vw] px-[5.6vw]">
        <div class="flex flex-row w-[50vw]  mx-auto bg-formColor p-4 rounded-lg drop-shadow-md">
            ...
            <div class="flex flex-col gap-3">
                ...
                <div class="mt-auto flex flex-row  ml-[15vw] gap-10">
                    <button class="border border-mainTextColor p-2 rounded-md hover:bg-buttonColor hover:border-buttonColor transform transition-colors duration-250 font-medium text-[16px] text-mainTextColor">Buy Now</button>
                    <form action="{{route('cart.add')}}" METHOD="POST">
                        @csrf
                        <input type="hidden" name="product_id" value="{{ $product->id }}">
                        <button class="border border-mainTextColor p-2 rounded-md hover:bg-buttonColor hover:border-buttonColor transform transition-colors duration-250 font-medium text-[16px] text-mainTextColor">Add to cart</button>
                    </form>
